...
void player1(int ground[GROUND_Y][GROUND_X], int *strategy_x, int *strategy_y){
	int out = 0;
	int sum=0;
	int def=0;
	int flag=0;
		...
		for( i=1, k=j ; (i<12-1, k<12-1) && flag==0 ; i++, k++){
			...
			for(int min=win-2;min>0;min--){
				if(def==win-min && ground[k+1][i+1]==0){
					...
					l=k+1;
					m=i+1;
					risk=player1_judge_side_effective(run_case,ground,def,l,m);
		...
		for( i=1, k=j ; (i<GROUND_X-1, k<GROUND_Y-1) && flag==0 ; i++, k++){
			...
			for(int min=win-2;min>0;min--){
				if(sum==win-min && ground[k+1][i+1]==0){
					...
					l=k+1;
					m=i+1;
					risk=player1_judge_side_effective(run_case,ground,sum,l,m);
				...
				for(i=1;i<GROUND_Y-1&&flag==0;i++){
					for(j=1;j<GROUND_X-1&&flag==0;j++){
						if(out==0 && (ground[i][j]!=p&&ground[i][j]!=0&&ground[i][j]!=-1)) {
							...
							sy=i+player1_N_case(rand()%3+1);
							sx=j+player1_N_case(rand()%3+1);
							if(ground[sy][sx]==0){
								//printf("NEUTRAL:%d,%d\n",sx,sy);
								//prot(&p,ground,sy,sx);
								*strategy_x=sx;
...
int player1_N_case(int player1_N_case){
	int move=0;
	switch(player1_N_case){
	case 1:
		move=1;
		break;
	case 2:
...
int player1_judge_side_effective(int run_case,int ground[GROUND_Y][GROUND_X],int dist,int l,int m){
	int plus;
	int d=dist+1;
	switch(run_case){
	case 1:
